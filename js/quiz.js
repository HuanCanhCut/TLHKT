document.addEventListener("DOMContentLoaded", () => {
  ...
  const quizQuestions = [
    {
      question:
        "What is the fundamental difference between laminar and turbulent flow?",
      options: [
      ...
      correctIdx: 1,
      explanation:
        "Laminar flow consists of smooth, parallel fluid layers, whereas turbulent flow is characterized by chaotic swirls and mixing.",
    },
    {
      question:
        "Which term refers to the mental effort required to process information in a learning task?",
      options: [
        "Reynolds number",
        "Spaced repetition",
        "Cognitive load",
    ...
    {
      question:
        "According to Cognitive Ergonomics, how should pilot displays be designed?",
      options: [
      ...
      correctIdx: 2,
      explanation:
        "Reducing extraneous cognitive load (distractions, messy layout) helps operators focus on essential safety information.",
    },
    {
      question:
        "What does transition to a high Reynolds number indicate in fluid dynamics?",
      options: [
      ...
      correctIdx: 0,
      explanation:
        "The Reynolds number is a dimensionless quantity that predicts the transition from laminar (low Re) to turbulent (high Re) flow.",
    },
    {
      question:
        "Which transition signal word is best used to state a contrast or counterpoint?",
      options: ["Crucially", "However", "In conclusion", "Importantly"],
      correctIdx: 1,
      explanation:
        "'However' is a transition word used to signal a contradiction or contrasting point.",
    },
  ];
  ...
  function renderQuestion() {
    ...
    quizQuestions[currentIdx].options.forEach((opt, idx) => {
      ...
      btn.className =
        "quiz-option group flex items-center p-md border border-outline rounded-xl hover:bg-surface-container-high transition-all text-left w-full cursor-pointer";
      btn.innerHTML = `
      ...
      btn.addEventListener("click", () => {
        ...
        document.querySelectorAll(".quiz-option").forEach((o) => {
          ...
          if (indicator) {
            indicator.className =
              "radio-indicator w-6 h-6 rounded-full border-2 border-outline mr-md flex-shrink-0 group-hover:border-primary flex items-center justify-center";
            indicator.innerHTML = "";
          }
        });
        ...
        btn.className =
          "quiz-option flex items-center p-md border-2 border-primary bg-primary/5 rounded-xl transition-all text-left w-full cursor-pointer";
        const indicator = btn.querySelector(".radio-indicator");
        if (indicator) {
          indicator.className =
            "radio-indicator w-6 h-6 rounded-full border-2 border-primary mr-md flex-shrink-0 flex items-center justify-center";
          indicator.innerHTML =
    ...
    nextBtn.addEventListener("click", () => {
      ...
      if (!isChecked) {
        ...
        if (selectedAns === correctIdx) {
          ...
          options[selectedAns].className =
            "quiz-option flex items-center p-md border-2 border-secondary bg-secondary/5 rounded-xl transition-all text-left w-full";
          const indicator =
            options[selectedAns].querySelector(".radio-indicator");
          if (indicator) {
            indicator.className =
              "radio-indicator w-6 h-6 rounded-full border-2 border-secondary mr-md flex-shrink-0 flex items-center justify-center";
            indicator.innerHTML =
          ...
          options[selectedAns].className =
            "quiz-option flex items-center p-md border-2 border-error bg-error/5 rounded-xl transition-all text-left w-full";
          const indicator =
            options[selectedAns].querySelector(".radio-indicator");
          if (indicator) {
            indicator.className =
              "radio-indicator w-6 h-6 rounded-full border-2 border-error mr-md flex-shrink-0 flex items-center justify-center";
            indicator.innerHTML =
              '<span class="material-symbols-outlined text-error text-sm" style="font-variation-settings: \'FILL\' 1">close</span>';
          }

          // Highlight correct answer
          options[correctIdx].className =
            "quiz-option flex items-center p-md border-2 border-secondary bg-secondary/5 rounded-xl transition-all text-left w-full";
          const correctIndicator =
            options[correctIdx].querySelector(".radio-indicator");
          if (correctIndicator) {
            correctIndicator.className =
              "radio-indicator w-6 h-6 rounded-full border-2 border-secondary mr-md flex-shrink-0 flex items-center justify-center";
        ...
        expBlock.className =
          "mt-md p-md bg-surface-container rounded-xl border border-outline-variant text-body-md text-on-surface-variant animate-fade-in";
        expBlock.innerHTML = `
